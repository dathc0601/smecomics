...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">Create Author</h1>
    <p class="text-gray-600">Add a new manga author</p>
</div>
    ...
    <form action="{{ route('admin.authors.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <textarea id="bio"
                      name="bio"
                      rows="4"
                      class="form-textarea @error('bio') border-red-500 @enderror">{{ old('bio') }}</textarea>
            @error('bio')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
