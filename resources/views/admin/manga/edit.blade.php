...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">Edit Manga</h1>
    <p class="text-gray-600">Update: {{ $manga->title }}</p>
</div>
    ...
    <form action="{{ route('admin.manga.update', $manga) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="form-group">
                    ...
                    <select id="author_id"
                        ...
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}" {{ old('author_id', $manga->author_id) == $author->id ? 'selected' : '' }}>
                                {{ $author->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="type"
                        ...
                        <option value="novel" {{ old('type', $manga->type) == 'novel' ? 'selected' : '' }}>Novel</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="number"
                           ...
                           value="{{ old('release_year', $manga->release_year) }}"
                           min="1900"
                           max="{{ date('Y') + 1 }}">
                    @error('release_year')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                <div class="form-group">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Upload new image (JPG, PNG, GIF - max 2MB)</p>
                    @error('cover_image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('translation_team', $manga->translation_team) }}">
                ...
                <div class="space-y-2">
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="is_featured" value="1" class="form-checkbox"
                            {{ old('is_featured', $manga->is_featured) ? 'checked' : '' }}>
                        <span class="text-sm">Featured</span>
                    </label>

                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="is_hot" value="1" class="form-checkbox"
                            {{ old('is_hot', $manga->is_hot) ? 'checked' : '' }}>
                        <span class="text-sm">Hot</span>
                    </label>

                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="is_18plus" value="1" class="form-checkbox"
                            {{ old('is_18plus', $manga->is_18plus) ? 'checked' : '' }}>
        ...
        <div class="form-group mt-6">
            ...
            <textarea id="description"
                      name="description"
                      class="tinymce @error('description') border-red-500 @enderror">{{ old('description', $manga->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
